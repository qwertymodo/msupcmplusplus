...
#include "soxconfig.h"
#include "sox_main.h"
#include "util.h" // re-include directly from sox to get #define'd min() and max()
...
sox_combine_method combine_method = sox_default;
sox_output_method output_method = sox_single;
#define is_serial(m) ((m) <= sox_concatenate)
#define is_parallel(m) (!is_serial(m))
sox_bool no_clobber = sox_false, interactive = sox_false;
sox_bool uservolume = sox_false;
lsx_enum_item const rg_modes[] = {
...
rg_mode replay_gain_mode = RG_default;
sox_option_t show_progress = sox_option_default;


/* Input & output files */

file_t * * files = NULL; /* Array tracking input and output files */
size_t file_count = 0;
...
*/
sox_effect_t **user_efftab = NULL;
size_t user_efftab_size = 0;
sox_effects_chain_t *effects_chain = NULL;
sox_effect_t *save_output_eff = NULL;

user_effargs_t **user_effargs = NULL;
size_t *user_effargs_size = NULL;  /* array: size of user_effargs for each chain */
...
size_t *nuser_effects = NULL;  /* array: number of effects in each chain */
size_t current_eff_chain = 0;
size_t eff_chain_count = 0;
sox_bool very_first_effchain = sox_true;
...
optimize_trim() hack. */
char *effects_filename = NULL;
char * play_rate_arg = NULL;
char *norm_level = NULL;

/* Flowing */

sox_signalinfo_t combiner_signal, ofile_signal_options;
sox_encodinginfo_t combiner_encoding, ofile_encoding_options;
uint64_t mixing_clips = 0;
size_t current_input = 0;
uint64_t input_wide_samples = 0;
uint64_t read_wide_samples = 0;
uint64_t output_samples = 0;
sox_bool input_eof = sox_false;
sox_bool output_eof = sox_false;
sox_bool user_abort = sox_false;
sox_bool user_skip = sox_false;
sox_bool user_restart_eff = sox_false;
int success = 0;
int cleanup_called = 0;
sox_sample_t omax[2], omin[2];

#ifdef HAVE_TERMIOS_H
#include <termios.h>
struct termios original_termios;
sox_bool original_termios_saved = sox_false;
#endif

sox_bool stdin_is_a_tty, is_player, is_guarded, do_guarded_norm, no_dither, reported_sox_opts;
...
{
	char const * const no_yes[] = { "no", "yes" };
	FILE * const output = sox_mode == sox_soxi ? stdout : stderr;
...
int output_flow(sox_effect_t *effp, sox_sample_t const * ibuf,
...
//void auto_effect(sox_effects_chain_t *, char const *, int, char **,
//	sox_signalinfo_t *, int *);

int add_effect(sox_effects_chain_t * chain, sox_effect_t * effp,
...
void auto_effect(sox_effects_chain_t *chain, char const *name, int argc,
...
double min_headroom = MIN_HEADROOM;

char const * vu(unsigned channel)
{
	struct timeval then;
	char const * const text[][2] = {
		/* White: 2dB steps */
		{ "", "" },{ "-", "-" },{ "=", "=" },{ "-=", "=-" },
		{ "==", "==" },{ "-==", "==-" },{ "===", "===" },{ "-===", "===-" },
		{ "====", "====" },{ "-====", "====-" },{ "=====", "=====" },
...
char const * const getoptstr =
"+b:c:de:hmnpqr:t:v:xBC:DGLMNRSTV::X";

struct lsx_option_t const long_options[] = {
...
lsx_enum_item const combine_methods[] = {
...
lsx_enum_item const endian_options[] = {
...
lsx_enum_item const plot_methods[] = {
...
{
	char const opts[] = "trcsdDbBpea?TV::";
	soxi_t type = Full;
